...
object Parboil {
  ...
  class JsonParser(val input: ParserInput) extends Parser with StringBuilding {
    ...
    def JsonObject: Rule1[JObject] = rule {
      ws('{') ~ zeroOrMore(Pair).separatedBy(ws(',')) ~ ws('}') ~> ((fields: Seq[(String, JValue)]) => JObject(fields.toList))
    }

    def Pair = rule { JsonStringUnwrapped ~ ws(':') ~ Value ~> ((_, _)) }

    def Value: Rule1[JValue] = rule {
    ...
    def JsonNumber = rule { capture(Integer ~ optional(Frac) ~ optional(Exp)) ~> (x => JNumber(x.toDouble)) ~ WhiteSpace }

    def JsonArray = rule { ws('[') ~ zeroOrMore(Value).separatedBy(ws(',')) ~ ws(']') ~> (x => JArray(x.toList)) }
